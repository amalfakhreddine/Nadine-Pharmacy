...
(function(){
  ...
  class Query{
    ...
    async get(){
      ...
      // Fetch the collection page-by-page so large SoftPharm catalogs (16k+) are fully loaded.
      const rows=[];
      const pageSize=1000;
      let from=0;
    ...
    onSnapshot(optionsOrOk,okOrFail,maybeFail){
      ...
      const load=()=>this.get().then(x=>{
        // Supabase queries here are server-backed.
        x.metadata={fromCache:false};
        if(alive&&typeof ok==='function')ok(x);
      }).catch(e=>{if(typeof fail==='function')fail(e)});
      load();
  ...
  window.nadineDb=db;
  window.nadineAuth=auth;
  window.sbFieldValue={serverTimestamp:()=>SERVER,delete:()=>DEL};
})();
